...
import type { JoinContent } from "@/data/club-landing-types";
...
export function SignUpForm({
  ...
  clubName: string;
  joinContent?: JoinContent | undefined;
  slug: string;
}): React.JSX.Element {
      ...
      <div className="grid min-h-0 w-full flex-1 grid-cols-1 lg:min-h-[calc(100dvh-5rem-5.5rem)] lg:grid-cols-[1fr_1fr]">
        <section className="group relative min-h-[360px] overflow-hidden lg:min-h-0">
          <div
            ...
            style={{
              backgroundImage:
                "url('/images/vanguard-vip-signup-reference.png')",
            }}
          />
            ...
            <div className="max-w-md pb-2 lg:pb-4">
              <div className="mb-5 flex items-center gap-3">
                ...
                <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                  {joinContent?.preHeadline ?? "The Vanguard Standard"}
                </p>
              </div>
              <h2 className="text-3xl font-semibold leading-tight text-foreground lg:text-4xl">
                {joinContent?.headline ?? "Access the Unattainable."}
              </h2>
              <p className="mt-4 text-base leading-relaxed text-muted-foreground lg:text-xl">
                {joinContent?.subheadline ??
                  "Membership is more than a status; it&apos;s a gateway to the world&apos;s most curated experiences and private circles."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="relative flex items-start justify-center border-t border-border-subtle bg-surface-deepest px-6 py-12 sm:px-10 lg:border-t-0 lg:bg-surface-deepest/95 lg:px-14 lg:pt-24">
          ...
          <div className="w-full max-w-[500px]">
            <h1 className="text-3xl font-semibold leading-tight text-foreground">
              {joinContent?.form?.headline ?? "Claim Your Place in the Club"}
            </h1>
            <p className="mt-3 text-lg leading-normal text-muted-foreground">
              {joinContent?.form?.subheadline ??
                "Welcome to a world of the extraordinary. Your membership starts with this step."}
            </p>
            ...
            <form className="mt-10 space-y-6" onSubmit={onSubmit}>
              ...
              <div>
                ...
                <Input
                  ...
                  {...form.register("email")}
                />
                <p className="text-xs text-form-error-text">
              ...
              {formError ? (
                <p className="text-sm text-form-error-text">{formError}</p>
              ) : null}
              {formMessage ? (
                <p className="text-sm text-form-success-note">{formMessage}</p>
              ) : null}
              ...
              >
                {form.formState.isSubmitting
                  ? "Creating account..."
                  : (joinContent?.form?.submitButtonLabel ?? "Join the Club")}
